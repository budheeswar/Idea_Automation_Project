...
/**
 * This suite is to test create, edit, delete dem_automation_document_de entry through UI.
 * 
...
public class TestWFSCrudUIDemAtmDocDESuite extends TestBase {
	...
	/**
	 * This method checks data insert/update/delete into Dem Automation Document De table from UI
	 *  It covers the following Test cases such as 
	 * 1)C907374 - Verify if the Manage dem_automation_document_de feature is available under in Security Management
	 * 2)C907375 - Create new Manage dem_automation_document_de details with correct format
	 * 3)C907376 - Verify View/Edit functionality for search Manage dem_automation_document_de
	 * 4)C907386 - Validate if Manage Dem Automation Document De table is created in the database
	 * 5)C907516 - Create new Manage dem_automation_document_de details with incorrect format
	 * 6)C907377 - Verify Delete functionality for Manage dem_automation_document_de
	 * 7)C907668 - Verify if Duplicate entries not allow in Manage Dem Automation Document De
	 */
	...
	public void TestManageDemAutomationDocumentDe(ITestContext iTestContext, HashMap<String, String> hashmap) {
		try {
			...
			if (hashmap.get("DemAutomationDocumentDeFunctionality").equals("Create")) {
				//Creating New Dem Automation Document De with document type id and client product id
				crudUIMaintenancePage.createNewDemAutomationDocumentDe(hashmap);
				Assert.assertTrue(SeleniumUtils.getValue(driver, crudUIMaintenancePage.successMessage).contains(hashmap.get("ExpectedResult"))
						);
				// validating the created documenttypeID, client productID into the dem automation Document De table
