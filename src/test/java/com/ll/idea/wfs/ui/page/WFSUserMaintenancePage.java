...
public class WFSUserMaintenancePage {
	...
	public void wFSMenuSelection(String mainMenu, String subMenu) {
		try {
		    ...
		    reportGenerator.logAndCaptureScreen("Navigation Link Screenshot Captured", "NavigationLink", Status.PASS, webDriver);
	...
	public void wFSMenuSelection(String mainMenu, String subMenu, ReportGenerator reportGenerator,String messages, String methodName, Status logLevel, WebDriver webDriver) {
		try {
			Reporter.log("IDEA Menu Selection");
			String sMenuXpath = "//*[@title='" + mainMenu + "']";
			WebElement mainMenuEle = webDriver.findElement(By.xpath(sMenuXpath));
			Actions action = new Actions(webDriver);
			action.moveToElement(mainMenuEle).build().perform();
			reportGenerator.logAndCaptureScreen(messages, methodName, logLevel, webDriver);
			CommonUtils.sleepForAWhile();
			SeleniumUtils.selectValueFromList(webDriver, iDEASubMenuLinks, subMenu);
		} catch (Exception e) {
			e.printStackTrace();
